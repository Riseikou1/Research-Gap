...
class SecurityRepository:
        ...
        with self.database.connect() as connection:
            ...
            connection.execute(
                """INSERT INTO accounts(user_id,email,email_verified,created_at,updated_at)
                   ...
                   email_verified=MAX(accounts.email_verified,excluded.email_verified),updated_at=excluded.updated_at""",
            ...
            if verified:
                connection.execute(
                    """INSERT OR IGNORE INTO credit_ledger
                       ...
                       VALUES(?,?,?,'grant','free_lifetime','lifetime','Verified free-account allowance',?)""",
                    (str(uuid4()), user_id, self.free_credits, now),
                )
    ...
    def reserve_credit(self, user_id: str, analysis_id: str) -> str:
        ...
        with self.database.connect() as connection:
            ...
            existing = connection.execute(
                "SELECT reservation_id FROM credit_reservations WHERE analysis_id=?", (analysis_id,)
            ).fetchone()
            if existing:
                connection.commit()
                return str(existing["reservation_id"])
            balance = connection.execute(
                "SELECT COALESCE(SUM(amount),0) AS value FROM credit_ledger WHERE user_id=?", (user_id,)
            ).fetchone()["value"]
            if int(balance) < 1:
                connection.rollback()
    ...
    def _finish_reservation(self, analysis_id: str, status: str, *, refund: bool) -> bool:
        ...
        with self.database.connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
                "SELECT * FROM credit_reservations WHERE analysis_id=?", (analysis_id,)
            ).fetchone()
    ...
    def record_rate_event(self, principal_key: str, action: str, *, limit: int, hours: int = 24) -> bool:
        ...
        with self.database.connect() as connection:
            ...
            count = connection.execute(
                "SELECT COUNT(*) AS count FROM rate_events WHERE principal_key=? AND action=? AND created_at>=?",
            ...
            if int(count) >= limit:
                ...
                return False
            connection.execute(
                "INSERT INTO rate_events VALUES(?,?,?,?)", (str(uuid4()), principal_key, action, now.isoformat())
            )
            connection.commit()
            return True
    ...
    def delete_account_data(self, user_id: str) -> None:
        ...
        with self.database.connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            connection.execute("DELETE FROM analyses WHERE owner_kind='user' AND owner_id=?", (user_id,))
            connection.execute(
                "UPDATE accounts SET email=NULL,display_name='',avatar_url=NULL,status='deleted',updated_at=? WHERE user_id=?",
    ...
    def set_status(self, admin_id: str, user_id: str, status: str, reason: str) -> None:
        ...
        with self.database.connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            connection.execute("UPDATE accounts SET status=?,updated_at=? WHERE user_id=?", (status, _now().isoformat(), user_id))
            ...
            if account and event_type == "invoice.paid" and self._invoice_has_price(obj, expected_price_id):
                ...
                connection.execute(
                    """INSERT INTO subscriptions(user_id,stripe_customer_id,stripe_subscription_id,status,current_period_end,last_event_created,updated_at)
                       ...
                       last_event_created=MAX(subscriptions.last_event_created,excluded.last_event_created),updated_at=excluded.updated_at""",
    ...
    def record_failed_payment_event(self, event: dict[str, object], error: str) -> None:
        ...
        with self.database.connect() as connection:
            connection.execute(
                """INSERT INTO payment_events(event_id,event_type,created_epoch,status,error_message,processed_at)
                   VALUES(?,?,?,'failed',?,?) ON CONFLICT(event_id) DO UPDATE SET
                   status='failed',error_message=excluded.error_message,processed_at=excluded.processed_at""",
                (event_id, str(event.get("type") or "unknown"), int(event.get("created", 0) or 0),
                 " ".join(error.split())[:500], _now().isoformat()),
            )
            connection.commit()
    ...
    def link_stripe_customer(self, user_id: str, customer_id: str) -> None:
        with self.database.connect() as connection:
            connection.execute("UPDATE accounts SET stripe_customer_id=?,updated_at=? WHERE user_id=?", (customer_id, _now().isoformat(), user_id))
            connection.commit()
