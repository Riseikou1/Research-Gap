...
function dedupePapers(papers:Paper[]){const seen=new Set<string>();return papers.filter(paper=>{const doi=paper.doi?.toLocaleLowerCase().replace(/^https?:\/\/(?:dx\.)?doi\.org\//,"").replace(/^doi:\s*/,"").replace(/\/$/,"");const key=doi?`doi:${doi}`:`title:${canonical(paper.title)}:${paper.publication_year??""}`;if(seen.has(key))return false;seen.add(key);return true})}
