...
export function ResultView({analysis}: {analysis: Analysis}) {
  ...
  const attempts = result.full_text_attempt_summary;
    ...
    <article className="report">
      ...
      {!isQuick && <ReportSection id="coverage" eyebrow="Reading depth" title="Evidence coverage">
        ...
        <div className="metrics coverage-metrics">
          ...
          <Metric value={extraction.full_text_successes} label="final full-text evidence records"/>
          <Metric value={fullTextRequested ? extraction.abstract_fallback_successes : extraction.abstract_successes} label={fullTextRequested ? "abstract fallback successes" : "abstract sources"}/>
        </div>
        {attempts&&<div className="metrics coverage-metrics">
          <Metric value={attempts.full_text_attempts} label="full-text access attempts"/>
          <Metric value={attempts.fetch_failures} label="fetch failures (attempt diagnostic)"/>
          <Metric value={attempts.parse_failures} label="parse failures (attempt diagnostic)"/>
          <Metric value={attempts.truncations} label="truncated full-text documents"/>
          <Metric value={attempts.model_schema_evidence_validation_failures} label="model/schema/evidence final failures"/>
          <Metric value={attempts.provider_failures} label="provider final failures"/>
        </div>}
        ...
        {result.paper_coverage.length > 0 && <details><summary>Per-paper coverage</summary><div className="evidence-records">{result.paper_coverage.map(record=><article className="evidence-card" key={record.paper_id}><h3>{record.title}</h3><p><code>{record.paper_id}</code> · {record.final_state === "success" ? pretty(record.final_evidence_level) : "Final extraction failure"}</p>{record.aliases.length > 1&&<p><strong>Canonical aliases:</strong> {record.aliases.join(", ")}</p>}<p><strong>Full text:</strong> {record.full_text_requested ? record.full_text_attempted ? `attempted${record.full_text_source_format ? ` as ${record.full_text_source_format.toUpperCase()}` : ""}` : "requested but no supported location was attempted" : "not requested"} · {pretty(record.full_text_status)}</p>{record.inspected_section_types.length > 0&&<p><strong>Full-text sections supplied to the extraction attempt:</strong> {record.inspected_section_types.map(pretty).join(", ")}.</p>}{record.truncated&&<p>Inspected content was truncated to the configured safety limit.</p>}{record.fallback_explanation&&<p>{record.fallback_explanation}</p>}</article>)}</div></details>}
...
function normalizedDoi(value:string|null|undefined){return value?.toLocaleLowerCase().trim().replace(/^https?:\/\/(?:dx\.)?doi\.org\//,"").replace(/^doi:\s*/,"").replace(/\/$/,"")??""}
function authorTokens(value:string){const tokens=value.normalize("NFKD").replaceAll(/\p{M}/gu,"").toLocaleLowerCase().match(/\p{L}+/gu)??[];return new Set(tokens)}
function authorMatch(left:Set<string>,right:Set<string>){const common=[...left].filter(token=>right.has(token));return common.length>=2&&common.length/Math.max(left.size,right.size)>=2/3}
function strongAuthors(left:Paper,right:Paper){const leftNames=left.authors.map(authorTokens).filter(tokens=>tokens.size>=2);const rightNames=right.authors.map(authorTokens).filter(tokens=>tokens.size>=2);const used=new Set<number>();let matches=0;for(const name of leftNames){const index=rightNames.findIndex((candidate,i)=>!used.has(i)&&authorMatch(name,candidate));if(index>=0){used.add(index);matches++}}return matches>=2||(matches===1&&leftNames.length===1&&rightNames.length===1)}
function sameDisplayWork(left:Paper,right:Paper){const leftDoi=normalizedDoi(left.doi);const rightDoi=normalizedDoi(right.doi);if(leftDoi&&leftDoi===rightDoi)return true;if(left.publication_year!==right.publication_year||canonical(left.title)!==canonical(right.title))return false;return !leftDoi||!rightDoi||strongAuthors(left,right)}
function dedupePapers(papers:Paper[]){const result:Paper[]=[];for(const paper of papers){if(!result.some(existing=>sameDisplayWork(existing,paper)))result.push(paper)}return result}
...
function Papers({papers}:{papers:Paper[]}){const[query,setQuery]=useState("");const[sort,setSort]=useState("relevance");const visible=dedupePapers(papers).filter(p=>p.title.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>sort==="year"?(b.publication_year??0)-(a.publication_year??0):(b.final_score??0)-(a.final_score??0));return <><div className="paper-tools"><label>Filter papers<input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search titles"/></label><label>Sort by<select value={sort} onChange={event=>setSort(event.target.value)}><option value="relevance">Relevance</option><option value="year">Newest</option></select></label></div><div className="paper-list">{visible.map(paper=><article className="paper" key={paper.id}><p className="paper-meta">{paper.publication_year??"Year unavailable"} · relevance {typeof paper.final_score==="number"?paper.final_score.toFixed(2):"not scored"}</p><h3>{paper.title}</h3><p>{paper.authors.join(", ")||"Authors unavailable"}</p>{paper.abstract&&<p>{paper.abstract.slice(0,560)}</p>}<div className="actions">{paper.url&&<a className="button secondary" href={paper.url} target="_blank" rel="noreferrer">Open paper</a>}{paper.doi&&<a className="button quiet" href={`https://doi.org/${paper.doi.replace("https://doi.org/","")}`} target="_blank" rel="noreferrer">View DOI</a>}</div>{paper.openalex_aliases.length>1&&<p className="hint"><strong>Canonical OpenAlex aliases:</strong> {paper.openalex_aliases.join(", ")}</p>}<details><summary>Retrieval provenance</summary><pre>{JSON.stringify(paper.provenance,null,2)}</pre></details></article>)}</div></>}
