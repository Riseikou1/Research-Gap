...
type Plan = {price_usd: number; interval: string; credits_per_cycle: number; test_mode: boolean; configured: boolean; billing_enabled: boolean; guest_quick_limit: number; user_quick_daily_limit: number; free_lifetime_credits: number};
const rows = [
...
export default function Pricing() {
  ...
  const billingAvailable = plan?.billing_enabled === true && plan.configured;
  ...
  return <div className="page pricing-page"><div className="page-head"><div><p className="eyebrow">Plans and allowances</p><h1>Choose the depth you need.</h1><p>One credit means one submitted Full Gap Analysis, including its bounded search, evidence extraction, landscape, gap candidates, and verification. Failed analyses return reserved credits.</p></div>{me?.signed_in&&<div className="credit-balance"><strong>{me.credit_exempt ? "Admin access" : me.credits}</strong><span>{me.credit_exempt ? "account credits not charged" : "credits remaining"}</span></div>}</div>
    <p className="test-banner" role="note">{billingAvailable ? "TEST / DEMO PRICING — NOT APPROVED FOR LIVE SALES" : "PAID SUBSCRIPTIONS ARE TEMPORARILY UNAVAILABLE"}</p>
    <div className="plan-cards">
      ...
      <article className="featured-plan"><p className="eyebrow">Placeholder plan</p><h2>Paid researcher</h2><p className="plan-price">{plan?`$${plan.price_usd}`:"…"}<small> / {plan?.interval??"month"}</small></p><p><strong>{plan?.credits_per_cycle??"…"} credits</strong> after each successfully paid test billing cycle, plus saved history and full-analysis features.</p>{billingAvailable ? <><p>Cancel or manage the test subscription through Stripe’s customer portal. Final price and credit amount will be chosen before live billing.</p>{session?<div className="actions"><button className="button primary" disabled={Boolean(busy)} onClick={()=>open("/billing/checkout")}>{busy==="/billing/checkout"?"Opening…":"Open test checkout"}</button><button className="button secondary" disabled={Boolean(busy)} onClick={()=>open("/billing/portal")}>Manage subscription</button></div>:<Link className="button primary" href="/sign-in">Sign in for test checkout</Link>}</> : <><p>New paid subscriptions cannot be purchased right now. Free accounts and existing credits remain available.</p><button className="button primary" type="button" disabled>Subscriptions unavailable</button></>}</article>
    </div>
