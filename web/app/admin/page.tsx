...
type Summary = {user_count:number; analyses:Array<{status:string;mode:string;count:number}>; failed_payment_events:Array<Record<string,unknown>>; audit_log:Array<Record<string,unknown>>; operations?:Record<string,unknown>};
...
export default function Admin() {
  ...
  return <div className="page"><div className="page-head"><div><p className="eyebrow">Owner operations</p><h1>Admin dashboard</h1><p>Account, usage, analysis, subscription, webhook, and audit summaries. Private research text is omitted from this broad view.</p></div><strong>{summary?.user_count ?? 0} users</strong></div>
    ...
    <section><h2>Operational diagnostics</h2><pre>{JSON.stringify(summary?.operations ?? {},null,2)}</pre></section>
